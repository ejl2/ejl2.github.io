...
Our testing process began with a brief explanation of what the purpose of our app was and the goal of the test itself. While we did not provide our users with a formal consent form, we did make sure to communicate to them that 1.) their participation is completely voluntary and they can leave at any point, 2.) we are testing how well the prototype intuitively helps the user complete certain tasks, 3.) this is in no way an evaluation of their ability. After this, we introduced the paper prototype to the user and explained that it is intended to mimic the interface of a mobile application where the user has the ability to tap on elements to select, as well as swiping and typing as appropriate. We informed the user that they are encouraged to talk out their actions, and that no observation is too trivial to mention, as we are interested in the unique insights they might have, especially those about confusing parts of our application flow. We especially encouraged them to ask questions, but to be mindful of the fact that we will likely not be able to answer them unless they are critical to the continuance of the test. Two of our users, Steve Montano and Strawberry Blast, represent people in in our user base who have experience with music individually, but lack the network or experience that come from playing often with multiple people. Barry Baritione represented a potential user who perhaps has more experience and might already have a band (two) that they are trying to manage.
...
### 8.) Digital Mockup
In all, the HiFi prototype faithfully captures all of the functionality captured in the LoFi prototype. Changes made mainly reflect stylistic decisions about color and spacing in order to make some functions more obvious or simply more attractive. A notable change was the inclusion of various "tip" cards that give some insight on how tasks might be useful to them. In addition, the profile page now does a better job of conveying the state of the apllication (ex. looking for matches) based on the elements used to submit one's profile rather than explicitly writing it out to the user as the paper prototype does.  
...
### 9.) Discussion
Overall, the testing and iterative design process showed us that from the beginning, our problem "themes" we identify stay roughly the same, however it is often the way we try to fix problems that changes between iterations. For example, while there are certainly issues that came up in some tests that did not come up in others, we saw that the salient issues generally fell into the categories discusssed in the results section. Throughout our testing we found that our application assumes too much: we lacked critical intermediate pages/steps that convey important information about the affordances of the application. In our case, the inclusion of these intermediate steps change our tasks by adding the ability to look through and compare multiple matches and manage multiple bands. The data entry portion was not as fleshed out as it needed to be, leading to confusion about what is indeed possible with drop downs, and uncertainty about the state of the application. In the paper prototype we attempted to address this by explicitly writing out to the user the state of their profile (ie submitted or not) and suggesting next actions. Through our iterative process, this revision eventually evolved to be less focused on explicitly writing things out to the user, but presenting elements within the digital mockup such that some of this information is just intuitive to the user. We definitely believe that the iterative design process could be continued from this point through usability tests with our high fidelity digital mockup. With the increased detail afforded to us with this new format, there are bound to be new usability issues that perhaps were not as obvious with the "higher level" experience of the paper prototypes. 
